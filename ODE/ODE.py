...
def solve_loop(
    initial_time, final_time, time_step, integration_time_step, problem, solver, x
):
...
#tqdm is used to show a progress bar for the loop
    for step in tqdm(range(1, time_steps_count + 1), desc="Getting numerical solution"):
        ...
        time, x, success = solver.solve(
            integration_time_step, current_stop_time, time, problem, x
        )
        if not success:
