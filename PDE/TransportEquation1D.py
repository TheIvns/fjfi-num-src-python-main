...
class ExplicitTransportEquationProblem1D:
    ...
    def function_f(self, t, u):
        ...
        if scheme == "fdm":
            for i in range(1, self.size - 1):
                fu[i] = -velocity * (u[i + 1] - u[i - 1]) / (2 * self.h)
            ...
            for i in range(1, self.size - 1):
                if velocity > 0:
                    fu[i] = -velocity * (u[i] - u[i - 1]) / self.h
                else:
                    fu[i] = -velocity * (u[i + 1] - u[i]) / self.h
        return fu
    ...
    def plot_solution(self, steps, size, solutions, save_fig=False):
        ...
        plt.xlabel("Position")
        plt.ylabel("Mass")
        plt.title("Transport Equation 1D Solution")
        plt.legend()
        plt.grid(True)
        if save_fig:
            plt.savefig(f"Transport-equation.png")
        plt.show()


if __name__ == "__main__":

    problem = ExplicitTransportEquationProblem1D(size, L)
